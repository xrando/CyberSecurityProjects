...
class audioSteg:
    ...
    def lsb_deinterleave_bytes(self, carrier, num_lsb, byte_depth=1):
        """
        ...
        """
        carrier_dtype = self.byte_depth_to_dtype[byte_depth]
        ...
        plen = self.roundup(num_bits / num_lsb)
    ...
    def lsb_interleave_bytes(
        self, carrier, payload, num_lsb, extPayload, truncate=False, byte_depth=1
    ):
        """
        ...
        bit_height = self.roundup(plen * 8 / num_lsb)
    ...
    def encode(self, audio_path, output_path, payload_path, num_lsb):
        print("\nEncoding Starts..")
        try:
            # Check if the file is MP3 or MP4 (lossly audio format)
            if audio_path.lower().endswith(".mp3") or audio_path.lower().endswith(".mp4"):
                audio_path = self.convert_audio_to_wav(audio_path, output_path)
            ...
            audio_frames = self.lsb_interleave_bytes(
    ...
    def decode(self, audio_path, output_path, num_lsb):
        print("\nDecoding Starts..")
        ...
        try:
            ...
            data, fileExt = self.lsb_deinterleave_bytes(
